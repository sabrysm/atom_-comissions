...
public class LootSplitConfirmCommand implements ISlashSubCommand {
    ...
    public void execute(@NotNull SlashCommandInteractionEvent event) {
        try {
            ...
            sendAmountAddedToAllPlayers(event, lootSplitInfo, amountToSplit);
    ...
    public void sendAmountAddedToAllPlayers(SlashCommandInteractionEvent event, OrderedMap<String, Object> lootSplitInfo, double amount) throws SQLException {
        List<MessageEmbed> pages = getPaginatorEmbed(event, lootSplitInfo, amount);
        long userId = event.getInteraction().getUser().getIdLong();
        long guildId = event.getGuild().getIdLong();
        LootSplitSubmissionPaginator paginator = new LootSplitSubmissionPaginator(pages, userId, guildId, lootSplitInfo.get("split_id").toString(), amount);
    ...
    public static List<MessageEmbed> getPaginatorEmbed(SlashCommandInteractionEvent event, OrderedMap<String, Object> lootSplitInfo, double amount) throws SQLException {
        ...
        firstPage.setTitle("Loot Split Submission");
